...
layui.define(["jquery","laytpl","layer"], function (exports) {
    ...
    Cascader.prototype={
        ...
        initOption: function () {
            ...
            if(self.option.canClickParent==undefined || self.option.canClickParent==true){
                self.canClickParent=true;
            }else{
                self.canClickParent=false;
            }
        ...
        callback: function () {
            ...
            self.domContent.on(self.triggerType,".urp-cascader-child li",function(event,triggerData){
                ...
                if("children" in self.blockData){
                    // 初始化子层
                    self.initChild(triggerData)

                    //判断是否支持点击父节点
                    if(self.canClickParent!=undefined && self.canClickParent==true){
